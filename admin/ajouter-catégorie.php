...
<section id="content">
    ...
    if (isset($_POST['ajouter'])) {
        ...
        if (!empty($_FILES['image'])) {
            ...
            move_uploaded_file($_FILES['image']['tmp_name'], '../upload/catégorie/' . $filename);
        }

        if (!empty($libelle) && !empty($description)) {
            require_once 'connexion.php';
            $sqlState = $pdo->prepare('INSERT INTO categorie(libelle,description,image,mesure) VALUES(?,?,?,?)');
            $sqlState->execute([$libelle, $description, $filename, $mesure]);
           // header('location:catégories.php');
        } else {
            ?>
            <div role="alert">
                Libellé, description sont obligatoires
            </div>
            <?php
        }
    }
    ...
    <main class="container">
        <header class="header">Ajouter Catégorie</header>
        ...
        <form method="post" class="form" enctype="multipart/form-data">
            ...
            <button name="ajouter">Ajouter Catégorie</button>
        </form>
    </main>
    <!-- MAIN -->
</section>
